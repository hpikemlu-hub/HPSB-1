 ...
 * Run: npx ts-node qa-testing/test-scripts/verify-integration.ts
 */
...
async function verifyIntegration() {
  ...
  try {
    ...
    if (partError) {
      ...
      withParticipants?.forEach((event: any) => {
    ...
    if (pastError) {
      ...
      past?.forEach((trip: any) => {
        const allCompleted = trip.calendar_todos?.every((ct: any) => ct.auto_completed);
