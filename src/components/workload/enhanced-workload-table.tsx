...
const PAGE_SIZE_OPTIONS = [10, 15, 35, 50, 100] as const;
...
export function EnhancedWorkloadTable({ 
  ...
  // Page.tsx already handles filtering - just sort here
  const filteredWorkloads = useMemo(() => {
    // Remove double filtering - workloads prop is already filtered by page
    return [...workloads].sort((a, b) => {
      const dateA = a.created_at ? new Date(a.created_at).getTime() : 0;
      const dateB = b.created_at ? new Date(b.created_at).getTime() : 0;
      return dateB - dateA; // Newest first
    });
  }, [workloads]); // Remove filters dependency

  // Update total items when workloads change
  useMemo(() => {
    setTotalItems(workloads.length); // Use original data length
  }, [workloads.length]);

  // Pagination calculations - Use ORIGINAL workloads length for total pages calculation
  // This ensures pagination shows even when filteredWorkloads is temporarily empty
  const totalPages = Math.ceil(workloads.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, filteredWorkloads.length);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg border">
          Menampilkan {startIndex + 1}-{endIndex} dari {filteredWorkloads.length} data
        </div>
      </div>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden">
        ...
        {/* Enhanced Pagination - Defensive UX: Show when ANY data exists */}
        {(filteredWorkloads.length > 0 || workloads.length > 0) && (
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm text-slate-600 hidden sm:block">
                ...
                <span className="text-slate-500 ml-2">
                  ({filteredWorkloads.length} total workload)
                </span>
              </div>
              ...
              <div className="flex items-center space-x-2">
                {/* Navigation buttons - Always show when we have workloads data */}
                {workloads.length > 0 && totalPages > 1 && (
                  <>
                    {/* First Page */}
                    <Button
                      variant="outline"
                ...
                {/* Page Numbers - Show when we have multiple pages */}
                {workloads.length > 0 && totalPages > 1 && (
                ...
                {/* Next/Last buttons - Always show when data exists */}
                {workloads.length > 0 && totalPages > 1 && (
                  <>
                    {/* Next Page */}
                    <Button
