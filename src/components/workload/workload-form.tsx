...
export function WorkloadForm({ mode, defaultValues, onSubmit, onCancel, isSubmitting, currentUser }: WorkloadFormProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in-0 slide-in-from-bottom-4 duration-700">
      {/* Enhanced Header Card with Gradient Animation */}
      <Card className="border-0 shadow-2xl bg-gradient-to-br from-white via-blue-50/30 to-indigo-100/40 overflow-hidden relative">
        ...
        <CardHeader className="pb-6 relative z-10">
          <div className="flex items-center space-x-6">
            <div className="relative">
              <div className="bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 p-4 rounded-2xl shadow-xl transform hover:scale-105 transition-all duration-300">
            ...
            <div className="flex-1">
              <CardTitle className="text-2xl font-bold text-slate-800 flex items-center gap-3 mb-2">
                {mode === 'create' ? 'Form Workload Baru' : 'Edit Workload'}
                {mode === 'create' && <Sparkles className="w-6 h-6 text-amber-500 animate-pulse" />}
              </CardTitle>
              <p className="text-slate-600 text-lg leading-relaxed">
                {mode === 'create' 
                  ? 'Lengkapi informasi workload dengan detail yang akurat'
                  : 'Perbarui informasi workload dan lacak progres dengan efisien'
                }
              </p>
            </div>
          </div>
